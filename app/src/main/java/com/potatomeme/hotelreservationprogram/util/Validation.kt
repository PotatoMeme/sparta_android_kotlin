...
import com.potatomeme.hotelreservationprogram.util.Utils.isValidDate
...
class Validation {
    ...
    fun showSelectCheckInDate(roomNum:Int): String {
        ...
        while (true) {
            ...
            if (isValidDate(date)){
                if (now > date) {
                    System.err.println("채크인은 지난날을 선택할수 없습니다")
                } else if (Service.getInstance().checkCheckInDate(roomNum, date)){
                    return date
                } else {
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    fun showSelectCheckInDate(roomNum:Int,ignorIdx:Int): String {
        val now: String = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd"))
        while (true) {
            println("체크인 날짜를 입력해주세요. 20230631")
            val date = readLine()!!
            if (isValidDate(date)){
                if (now > date) {
                    System.err.println("채크인은 지난날을 선택할수 없습니다")
                } else if (Service.getInstance().checkCheckInDate(roomNum, date,ignorIdx)){
                    return date
                } else {
                    System.err.println("해당 날짜에 이미 방을 사용중입니다. 다른 날짜를 입력해주세요")
                }
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    fun showSelectCheckOutDate(roomNum: Int,beforeDate: String) : String {
        ...
        while (true) {
            ...
            if (isValidDate(date)){
                if (beforeDate >= date) {
                    System.err.println("체크인 날짜보다 이전이거나 같을수 없습니다")
                }else if (Service.getInstance().checkCheckOutDate(roomNum, beforeDate, date)){
                    return date
                } else {
                    System.err.println("해당 날짜에 이미 방을 사용중입니다. 다른 날짜를 입력해주세요")
                }
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    fun showSelectCheckOutDate(roomNum: Int,beforeDate: String,ignorIdx: Int) : String {
        require(isValidDate(beforeDate)){"beforeData($beforeDate) is not yyyyMMdd. check again"}
        while (true) {
            println("체크아웃 날짜를 입력해주세요. 20230631")
            val date = readLine()!!
            if (isValidDate(date)){
                if (beforeDate >= date) {
                    System.err.println("체크인 날짜보다 이전이거나 같을수 없습니다")
                }else if (Service.getInstance().checkCheckOutDate(roomNum, beforeDate, date,ignorIdx)){
                    return date
                } else {
                    System.err.println("해당 날짜에 이미 방을 사용중입니다. 다른 날짜를 입력해주세요")
                }
